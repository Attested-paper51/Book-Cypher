  ...
  {                                                                                                            //passed with the header node, the number being searched, index of array, and a PrintStream variable
    ...
    while(p.next != null)
    {
      ...
      if(p.next.item == number)
      {
        if(index == 10)
        {
          output.println(index);  //prints index in a .txt file following a jump to the next line - will happen whenever a char from "message.txt" represents a new line
        }
        else
        {
          output.print(index + "  ");  //prints index in a .txt file
        }
        break;
      }
      else
